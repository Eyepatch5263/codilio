...
import { LANGUAGE_CONFIG } from "../_constants/page";
import { AnimatePresence, motion } from "framer-motion";
import { ChevronDownIcon, Lock, Sparkles } from "lucide-react";
import Image from "next/image";
import useMounted from "@/hooks/useMounted";

const LanguageSelector = ({ hasAcess }: { hasAcess: boolean }) => {
  const [isOpen, setIsOpen] = useState(false);
  const { language, setLanguage } = useCodeEditorStore();
  const dropdownRef = useRef<HTMLDivElement>(null);
  const selected_language = LANGUAGE_CONFIG[language];
  ...
  const handleLanguageSelect = (langId: string) => {
    if(!hasAcess && langId !== "javascript") return;
    setLanguage(langId);
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      ${!hasAcess && language !== "javascript" ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        {/* Decoration */}
        <div
        ...
        <div className="size-6 rounded-md bg-gray-800/50 p-0.5 group-hover:scale-110 transition-transform">
          <Image
            src={selected_language.logoPath}
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                const isLocked = !hasAcess && lang.id !== "javascript";
