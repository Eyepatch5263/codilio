...
import { THEMES } from '../_constants/page'
    ...
    return (
        <div className='relative' ref={dropdownRef}>
            <motion.button
                ...
                className='w-48 group relative flex items-center gap-2 px-4 py-2.5 bg-[#1e1e2e]/80 hover:bg-[#262637] rounded-xl transition-all duration-200 border border-gray-800/50 hover:border-gray-700 '
            >
                {/* decorator*/}
                <div className="flex inset-0 bg-gradient-to-r from-blue-500/5 to-purple-500/5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity" />
                <Palette className="w-4 h-4 text-gray-400 group-hover:text-gray-300 transition-colors" />
                <span className='text-gray-300 min-w-[80px] text-left group-hover:text-white transition-colors'>{currentTheme?.label}</span>

                <div className='relative w-4 h-4 rounded-full border border-gray-400 group-hover:border-gray-500 transition-colors' style={{ background: currentTheme?.color }} />
            </motion.button>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
